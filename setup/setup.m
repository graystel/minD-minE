function [im, branchData, pixeltoVertex, adjacencyMatrix, edgeMatrix] = setup(filepath, loopremover, N_ims, i_tstart, N_frames, dt, reset, hyphaeThicknessRange, minArea, removeloop)
    %housekeeping
    arguments 
        filepath 
        loopremover
    ...
    totalPlanes = max([object.sizeT, object.sizeZ]);  %TIF and BTF files could store info differently (along t-dim or z-dim)
    if isempty(N_ims) || N_ims > (totalPlanes - i_tstart) %if the number of frames is not useful
        N_ims = totalPlanes - i_tstart; %redfine 
    end

    firstFrame = getPlane(object, 1, 1, 1);
    [Ny, Nx, ~] = size(firstFrame);
    im = zeros(Ny, Nx, N_ims);
    ...
    if ~exist('im', 'var') || ~exist('imframe', 'var') %i.e. if a variable named im or imframe don't exist, then we want to prepare them
        for i_t = 1:N_ims %for loop through all the frames
            planeIdx = i_tstart + i_t; 
            if object.sizeT >= planeIdx %if the T-dim size is greater than 
                imframe = getPlane(object, 1, 1, planeIdx); %gets current frame from the btf file
            else
                imframe = getPlane(object, planeIdx, 1, 1); % fallback to Z-plane
            end
    ...
    vcc = bwconncomp(isolatedVertices, 8);
    numVertices = vcc.NumObjects;
    ...
    pixeltoVertex = zeros(Ny, Nx);
    for i=1:numVertices
        pixeltoVertex(vcc.PixelIdxList{i}) = i; %inside pixeltoVertex, make all pixels associated with the vth ccomp in VCC have value i
    end

    adjacencyMatrix = zeros(numVertices, numVertices, 0); %for keeping track of branch between 2 verts
    edgeMatrix = zeros(cc.NumObjects, 3); %for keeping track of the 2 verts for each branch
    ...
    for i = 1:cc.NumObjects
        ...
        windowsize = 5;
        xwindowstart = max(1, xstart-windowsize) : min(Nx, xstart+windowsize);
        xwindowend = max(1, xend-windowsize) : min(Nx, xend+windowsize);
        ywindowstart = max(1, ystart-windowsize) : min(Ny, ystart+windowsize);
        ywindowend = max(1, yend-windowsize) : min(Ny, yend+windowsize);

        possiblevert1 = pixeltoVertex(ywindowstart, xwindowstart);
        possiblevert2 = pixeltoVertex(ywindowend, xwindowend);

        vert1 = max(possiblevert1(:));
        vert2 = max(possiblevert2(:));

        if vert1 > 0 && vert2 > 0
            existinglayers = squeeze(adjacencyMatrix(vert1, vert2, :));
            freelayer = find(existinglayers == 0, 1, 'first');
            
            if isempty(freelayer)
                freelayer = size(adjacencyMatrix, 3) + 1;
            end

            adjacencyMatrix(vert1, vert2, freelayer) = i;
            adjacencyMatrix(vert2, vert1, freelayer) = i;
            edgeMatrix(i,:) = [i vert1 vert2];
        end
    end

    if removeloop == true
        [branchData, adjacencyMatrix, edgeMatrix] = loopremover(branchData, adjacencyMatrix, edgeMatrix);
    end
